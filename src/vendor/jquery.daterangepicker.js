...
export default function($, moment) {
  ...
  $.fn.dateRangePicker = function(opt) {
    ...
    function init_datepicker() {
      ...
      box
        .find('.default-top')
        .html(defaultTopText.replace(/\%d/, opt.minDays).replace(/\%d/, opt.maxDays));
      if (opt.singleMonth) {
      ...
      box.find('.apply-btn').click(function() {
        closeDatePicker();
        var dateRange =
          getDateString(new Date(opt.start)) + opt.separator + getDateString(new Date(opt.end));
    ...
    function showSelectedDays() {
      ...
      box.find('.day').each(function() {
        ...
        if (
          ...
          (opt.start &&
            !opt.end &&
            moment(start).format('YYYY-MM-DD') == moment(time).format('YYYY-MM-DD'))
        ) {
    ...
    function generateMonthElement(date, month) {
      ...
      var startDate = opt.startDate
        ? moment(opt.startDate).add(!opt.singleMonth && month === 'month2' ? 1 : 0, 'month')
        : false;
      var endDate = opt.endDate
        ? moment(opt.endDate).add(!opt.singleMonth && month === 'month1' ? -1 : 0, 'month')
        : false;
      date = moment(date);

      if (!opt.monthSelect || (startDate && endDate && startDate.isSame(endDate, 'month'))) {
    ...
    function generateYearElement(date, month) {
      ...
      if (!opt.yearSelect || (startDate && endDate && startDate.isSame(moment(endDate), 'year'))) {
    ...
    function generateSelect(name, data) {
      ...
      for (var i = 0, l = data.length; i < l; i++) {
        select +=
          '<option value="' + data[i].value + '"' + (data[i].isCurrent ? ' selected' : '') + '>';
        select += data[i].text;
        select += '</option>';
        ...
        if (data) {
          ...
          if (data.prev && data.prev.length > 0) {
            ...
            for (var i = 0; i < data.prev.length; i++) {
              ...
              html +=
                ' <a href="javascript:;" shortcut="prev,' + data.prev[i] + '">' + name + '</a>';
            }
            html += '</span>';
          }
          ...
          if (data.next && data.next.length > 0) {
            ...
            for (var i = 0; i < data.next.length; i++) {
              ...
              html +=
                ' <a href="javascript:;" shortcut="next,' + data.next[i] + '">' + name + '</a>';
            }
            html += '</span>';
          }
        }
    ...
    function translate(translationKey) {
      ...
      var result =
        translationKey in languages
          ...
          : translationKeyLowerCase in languages ? languages[translationKeyLowerCase] : null;
